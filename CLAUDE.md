...
- `session_id` — random UUID per browser session (`sessionStorage`) — one "visit"
- `ip_hash` — pseudonymous visitor id, `HMAC-SHA256(SESSION_SECRET, req.ip)` (first 32 hex), stamped **server-side**. One-way — the raw IP is never stored, so no PII. Same IP → same hash → one "user". Enables user/visit/landing/returning metrics. See `server/analytics/identity.ts`.
...
| timestamp | session_id | event | route | referrer | device | read_seconds | ip_hash |
|-----------|------------|-------|-------|----------|--------|--------------|---------|

It was created with `npx tsx scripts/create-analytics-workbook.ts` (idempotent — creates the table if missing, and **reconciles columns** by appending any missing `EventRow` fields non-destructively, so it doubles as a schema migrator). Set `ANALYTICS_WORKBOOK_PATH` to target a throwaway workbook for local testing. The existing Graph API credentials already have `Files.ReadWrite` scope — no new permissions needed.
...
**No cookies, no banner, no PII.** Session ID lives in `sessionStorage` (cleared when the tab closes) — not a cookie. Referrer is the external hostname only (same-site navigations are ignored). `ip_hash` is a one-way keyed hash computed server-side (raw IP discarded immediately), so the workbook holds no personal data and the site needs no consent banner. Identity model: **user** = `ip_hash`, **visit** = `session_id`, **landing page** = first route per visit.
